item_attribute_amplifier_5x = class({})

LinkLuaModifier("modifier_attribute_amplifier_5x", "items/item_attribute_amplifier_5x", LUA_MODIFIER_MOTION_NONE)

function item_attribute_amplifier_5x:GetIntrinsicModifierName()
    return "modifier_attribute_amplifier_5x"
end

-- 基础物品属性设置
function item_attribute_amplifier_5x:Spawn()
    if IsServer() then
...
modifier_attribute_amplifier_5x = class({})

function modifier_attribute_amplifier_5x:IsHidden() return false end
function modifier_attribute_amplifier_5x:IsDebuff() return false end
function modifier_attribute_amplifier_5x:IsPurgable() return false end
function modifier_attribute_amplifier_5x:RemoveOnDeath() return false end
function modifier_attribute_amplifier_5x:IsPermanent() return true end

function modifier_attribute_amplifier_5x:DeclareFunctions()
    return {
...
function modifier_attribute_amplifier_5x:OnCreated()
    ...
    local target_total_str = self.original_total_str * 5
    local target_total_agi = self.original_total_agi * 5
    local target_total_int = self.original_total_int * 5
...
function modifier_attribute_amplifier_5x:GetModifierBonusStats_Strength()
    local hero = self:GetParent()
    if not hero:IsIllusion() then return 0 end
    return self.original_str * 4  -- 使用记录的原始属性计算
end

function modifier_attribute_amplifier_5x:GetModifierBonusStats_Agility()
    local hero = self:GetParent()
    if not hero:IsIllusion() then return 0 end
    return self.original_agi * 4
end

function modifier_attribute_amplifier_5x:GetModifierBonusStats_Intellect()
    local hero = self:GetParent()
    if not hero:IsIllusion() then return 0 end
    return self.original_int * 4
end

function modifier_attribute_amplifier_5x:PrintAttributeChanges()
    local hero = self:GetParent()
