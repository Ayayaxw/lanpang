modifier_attack_cast_ability_2 = class({})

function modifier_attack_cast_ability_2:IsHidden()
    return false
end

function modifier_attack_cast_ability_2:IsDebuff()
    return false
end

function modifier_attack_cast_ability_2:IsPurgable()
    return false
end

function modifier_attack_cast_ability_2:OnCreated()
    if not IsServer() then return end
    -- 启动定时器处理队列

    -- 定义特殊技能处理方式（共享是可以的，因为这只是规则定义）
    if not modifier_attack_cast_ability_2.special_abilities then
        modifier_attack_cast_ability_2.special_abilities = {
            -- 技能名称 = 处理方式
...
function modifier_attack_cast_ability_2:OnIntervalThink()
    if not IsServer() then return end
    self:ProcessQueue()
end

function modifier_attack_cast_ability_2:ProcessQueue()
...
function modifier_attack_cast_ability_2:DeclareFunctions()
    local funcs = {
...
function modifier_attack_cast_ability_2:OnAttackLanded(params)
    if not IsServer() then return end
    ...
    -- 获取单位的第一个技能（index 0）
    local ability = CommonAI:GetRandomSecondSkill(parent)
    ...
    elseif targetType == DOTA_UNIT_TARGET_TYPE.TREE then
        ...
        -- 检查是否是特殊处理的技能
        local special_behavior = modifier_attack_cast_ability_2.special_abilities[abilityName]
