function Main:Cleanup_Fall_Flat()


end

function Main:Init_Fall_Flat(event, playerID)
    ...
    CreateHero(playerID, heroName, selfFacetId, self.smallDuelAreaLeft, DOTA_TEAM_GOODGUYS, false, function(playerHero)
        ...
        if selfAIEnabled then
            Timers:CreateTimer(self.duration - 0.7, function()
                ...
                CreateAIForHero(self.leftTeamHero1)
                return nil
            end)
        else
        end
    ...
    CreateHero(playerID, opponentHeroName, opponentFacetId, self.smallDuelAreaRight, DOTA_TEAM_BADGUYS, false, function(opponentHero)
        ...
        if opponentAIEnabled then
            ...
            Timers:CreateTimer(self.duration - 0.7, function()
                if self.currentTimer ~= timerId or hero_duel.EndDuel then return end
                CreateAIForHero(self.rightTeamHero1)
                return nil
            end)
        else
        end
...
function Main:OnUnitKilled_Fall_Flat(killedUnit, args)
    ...
    local winningHero, losingHero
    local isLeftTeamWin = killedUnit == self.rightTeamHero1
...
function Main:OnNPCSpawned_Fall_Flat(spawnedUnit, event)
    -- 如果不是被排除的单位，则应用战场效果
