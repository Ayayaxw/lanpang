function Main:Init_Attack_Trigger_2skill(event, playerID)
    -- 基础参数初始化
    ...
    Timers:CreateTimer(self.duration, function()
        ...
        -- 开始生成花岗岩定时器
        self:StartSpawning_Attack_Trigger_2skill(timerId)
...
-- 花岗岩单位生成函数
function Main:StartSpawning_Attack_Trigger_2skill(timerId)
    -- 设置花岗岩单位名称
...
-- 单位死亡判定
function Main:OnUnitKilled_Attack_Trigger_2skill(killedUnit, args)
...
function Main:OnNPCSpawned_Attack_Trigger_2skill(spawnedUnit, event)

    spawnedUnit:AddNewModifier(spawnedUnit, nil,"modifier_death_check_enchant", {})

    if spawnedUnit:IsRealHero() then
        spawnedUnit:AddNewModifier(spawnedUnit, nil, "modifier_attack_cast_ability_2", {})
    end
