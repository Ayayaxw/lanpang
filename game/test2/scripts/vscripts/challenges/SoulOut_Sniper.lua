function Main:Init_SoulOut_Sniper(event, playerID)
    -- 技能修改器
    self.courierPool = {}
    ...
    Timers:CreateTimer(5, function()
        ...
        self:PrepareHeroForDuel(
            ...
            self.waterFall_Center,      --向南移动500码 
...
function PreSpawnSniper()
    local centerPoint = Main.waterFall_Center
    local radius = 600  -- 圆的半径
    local totalUnits = 5  -- 要创建的单位数量
    local angleStep = 360 / totalUnits  -- 单位之间的角度间隔
    ...
    for i = 1, totalUnits do
        -- 计算每个单位的位置
        local angleOffset = 90  -- 逆时针旋转90度，使起始点在正东方
        local angle = math.rad(i * angleStep + angleOffset)
        local spawnX = centerPoint.x + radius * math.cos(angle)
        local spawnY = centerPoint.y + radius * math.sin(angle)
        local spawnPosition = Vector(spawnX, spawnY, 128)
        
        local unit = CreateUnitByName("sniper", spawnPosition, true, nil, nil, DOTA_TEAM_BADGUYS)
        ...
        if unit then
            ...
            -- 计算朝向中心点的向量
            local directionToCenter = (centerPoint - spawnPosition):Normalized()
            ...
            --学习index为2的技能等级+1
            local ability = unit:FindAbilityByName("sniper_headshot")
            if ability then
                ability:SetLevel(1)
            end
    ...
    Timers:CreateTimer(8, function()
        for _, unit in pairs(spawned_units) do
            if unit and unit:IsAlive() then
                -- 应用AI
                CreateAIForHero(unit, {"不要优先拆墓碑、棒子"}, {"默认策略"}, "野怪AI", 0.1)
            end
        end
    end)
end

function Main:OnUnitKilled_SoulOut_Sniper(killedUnit, args)
    ...
    local survivalTime = GameRules:GetGameTime() - self.startTime
    local formattedTime = string.format("%02d:%02d.%02d", 
    math.floor(survivalTime / 60),
    math.floor(survivalTime % 60),
    math.floor((survivalTime * 100) % 100))
    local finalScore = hero_duel.killCount * 3 + math.floor(survivalTime)
    ...
    if killedUnit == self.leftTeamHero1 then
        ...
        --打印发送的时间
        print("[存活时间] 发送的时间:", formattedTime)
        --CustomGameEventManager:Send_ServerToAllClients("update_score", data)
    ...
    if killedUnit:GetUnitName() == "sniper" then
        ...
        if spawnPosition then
            Timers:CreateTimer(1, function()
                ...
                killedUnit:SetBaseMoveSpeed(200)
                ...
                -- 播放复活特效
                local particleID = ParticleManager:CreateParticle("particles/items_fx/aegis_respawn_timer.vpcf", PATTACH_ABSORIGIN_FOLLOW, killedUnit)
                ParticleManager:ReleaseParticleIndex(particleID)
                
                -- 播放复活音效
                EmitSoundOn("Hero_LoneDruid.BattleCry.Bear", killedUnit)
            end)
        end
    end
end

function Main:OnNPCSpawned_SoulOut_Sniper(spawnedUnit, event)
    -- 如果不是被排除的单位，则应用战场效果
