function Main:Init_Attack_Trigger_1skill(event, playerID)
    -- 基础参数初始化
    ...
    local order = {"挑战英雄", "击杀数量", "黑龙等级", "黑龙生命值", "黑龙攻击力", "剩余时间"}
    ...
    Timers:CreateTimer(self.duration, function()
        ...
        -- 开始生成黑龙定时器
        self:StartSpawning_Attack_Trigger_1skill(timerId)
...
-- 黑龙单位生成函数
function Main:StartSpawning_Attack_Trigger_1skill(timerId)
    -- 设置黑龙单位名称
    local dragonUnit = "npc_dota_neutral_black_dragon"
...
function Main:SpawnDragon(unitName, spawnPos, timerId)
    ...
    CustomGameEventManager:Send_ServerToAllClients("update_score", {
        ["黑龙生命值"] = tostring(currentHealth),
        ["黑龙攻击力"] = tostring(currentAttack)
    })
...
-- 单位死亡判定
function Main:OnUnitKilled_Attack_Trigger_1skill(killedUnit, args)
    ...
    if unitName == "npc_dota_neutral_black_dragon" then
        ...
        if hero_duel.killCount % 10 == 0 then
            ...
            CustomGameEventManager:Send_ServerToAllClients("update_score", {
                ["黑龙等级"] = tostring(hero_duel.dragonLevel)
            })
        end
        ...
        Timers:CreateTimer(0.2, function()
            ...
            self:SpawnDragon("npc_dota_neutral_black_dragon", spawnPos, self.currentTimer)
        end)
    end
end
