item_attribute_amplifier_3x = class({})

LinkLuaModifier("modifier_attribute_amplifier_3x", "items/item_attribute_amplifier_3x", LUA_MODIFIER_MOTION_NONE)

function item_attribute_amplifier_3x:GetIntrinsicModifierName()
    return "modifier_attribute_amplifier_3x"
end

-- 基础物品属性设置
function item_attribute_amplifier_3x:Spawn()
    if IsServer() then
...
function modifier_attribute_amplifier_3x:PrintAttributeChanges()
    local hero = self:GetParent()
    print("Attribute Amplifier (3x) for " .. hero:GetName())
